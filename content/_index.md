...
_Download counts, read every night from GitHub releases and Docker Hub, for every nightly package. Every way of getting Ansel — the download buttons, the in-app update, Homebrew, Scoop, AppImageUpdate — ends up in these counters. [All builds, per package, with their crash rates](/download/)._

{{% row %}}
{{% column %}}
{{% card %}}

{{< plotly title="Download statistics: downloads per month, all packages. Packages from GitHub releases are measured between two daily snapshots; before the daily record began (lighter bars) they are the downloads of the packages built that month. Docker Hub pulls, stacked on top, are measured the same way and exist only from the daily record on." caption="false" src="downloads-monthly.json" class="full-width" dynamic="true" >}}
...
{{< plotly title="Download statistics: share of each package format, all time (GitHub release assets, and Docker Hub pulls)." caption="false" src="downloads-formats.json" class="full-width" dynamic="true" >}}
